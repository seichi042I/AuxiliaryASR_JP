...
class CustomBatchSampler(Sampler):
    def __init__(self, data_source, batch_size,shuffle=False):
        ...
        self.indices.sort(key=lambda x: len(self.data_source[x][3]))  # 長さによってインデックスをソート

    def __iter__(self):
        batch = []
        length = len(self.indices)
        for i in range(length):
            batch.append(self.indices[i])
            batch.append(self.indices[length - i -1])
...
def build_dataloader(path_list,
    ...
    custom_sampler = CustomBatchSampler(dataset,batch_size)
